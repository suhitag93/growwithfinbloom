...
interface GardenData {
  emergencyMonths: number;   // 0–6+
  savingsRate: number;       // 0–100%
  debtLevel: number;         // 0–100 (100 = lots of debt)
  investmentGrowth: number;  // 0–100 milestone %
  habitStreak: number;       // days
}

// Mock data – replace with real data hooks later
const MOCK_DATA: GardenData = {
  emergencyMonths: 4,
  savingsRate: 35,
  debtLevel: 25,
  investmentGrowth: 60,
...
const Pond = ({ months, onTap }: { months: number; onTap: () => void }) => {
  ...
  return (
    <g
      ...
      role="button"
      aria-label="Emergency fund details"
    >
      <motion.ellipse
...
const Flower = ({ x, y, color, delay, onTap, size = 1 }: { x: number; y: number; color: string; delay: number; onTap: () => void; size?: number }) => (
...
const Tree = ({ x, y, height, onTap }: { x: number; y: number; height: number; onTap: () => void }) => {
...
const Weed = ({ x, y, opacity }: { x: number; y: number; opacity: number }) => {
...
const FinancialGarden = () => {
  const { currentLevel, totalXP, nextLevel, progress } = useXP();
    ...
    >
        ...
        >
          ...
          <defs>
            ...
            <linearGradient id="ground-grad" x1="0%" y1="0%" x2="0%" y2="100%">
              <stop offset="0%" stopColor="hsl(152, 25%, 82%)" />
              <stop offset="100%" stopColor="hsl(152, 15%, 88%)" />
            </linearGradient>
          </defs>
          ...
          {/* Seed stage – just soil and seed */}
          {stageIndex === 0 && <SeedScene />}

          {/* Weeds (visible when debt is high) */}
          {stageIndex >= 1 && (
            <g onClick={() => setActiveDetail(details.weeds)} className="cursor-pointer">
              <Weed x={340} y={278} opacity={weedOpacity} />
          ...
          {/* Pond / Water */}
          {showPond && <Pond months={data.emergencyMonths} onTap={() => setActiveDetail(details.pond)} />}
          ...
          {/* Butterflies */}
          {showButterflies && <Butterfly delay={2} />}
      ...
      <div className="px-4 pb-4 pt-2 md:px-6 md:pb-6">
        ...
        <motion.p
          ...
          animate={{ opacity: 1 }}
          transition={{ delay: 2 }}
          className="text-[11px] text-center text-primary/70 mt-3 font-medium"
        >
